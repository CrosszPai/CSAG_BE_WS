...
app.post('/api/user/login', (req, res) => {
    ...
    client.query(/* ใส่ query string ที่แสดงค่า user ที่มี username=$1จาก table account */, [username], (err, data) => {
        if (err) {
            res.status(401)
        }
...
app.get('/api/posts', async (req, res) => {
    client.query(/* ใส่ query string ให้แสดงค่าทั้งหมด ใน table posts */, (err, data) => {
        if (err) {
            console.log(err.stack)
        } else {
